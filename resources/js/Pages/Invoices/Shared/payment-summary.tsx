        ...
        <div className="w-full mx-auto bg-white  rounded-md">
            <ul className="space-y-1">
                {usedMethods.map((method, index) => (
                    <li
                        key={index}
                        className="p-3 bg-gray-50 rounded-md border border-gray-100 text-gray-700"
                    >
                        <div className="font-medium uppercase text-indigo-600">{method.method}</div>
                        <div>
                            <span className="font-semibold">${method.amount.toFixed(2)}</span>
                            {method.reference && (
                                <span className="text-sm text-gray-500 ml-2">(Ref: {method.reference})</span>
                            )}
                        </div>
                        {method.additionalInfo && (
                            <div className="text-sm text-gray-500">
                                {method.additionalInfo.brand && (
                                    <span>Brand: {method.additionalInfo.brand}, </span>
                                )}
                                {method.additionalInfo.last4 && (
                                    <span>Last4: {method.additionalInfo.last4}</span>
                                )}
                            </div>
                        )}
                    </li>
            ...
            {primaryMethod && (
                <div className="mt-2 p-4 bg-indigo-50 rounded-md border border-indigo-200">
