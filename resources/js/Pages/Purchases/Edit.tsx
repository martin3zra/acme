...
import { defaultDiscount, makeEditBreadcrumbs, paymentTerms, purchaseKindMeta } from './constants';
...
export default function Edit({
  ...
  const { put, transform, processing, errors } = useForm<PurchaseFormData>({
    vendor_id: 0,
    terms: 'pia',
    lines: [],
    date: new Date(),
  ...
  const initialAsPurchaseForm = (): PurchaseForm => {
    return {
      header: {
        vendor: purchase.header.vendor,
        date: new Date(purchase.header.date),
        due: purchase.header.due_on ? new Date(purchase.header.due_on) : undefined,
        terms: purchase.header.terms,
        notes: purchase.header.notes,
        discount: purchase.header.discount,
      },
  ...
  const [purchaseForm, setPurchaseForm, removePurchaseForm] = usePersistedState<PurchaseForm>(`purchase_${kind}_edit`, initialAsPurchaseForm(), true);
  ...
  const handleVendorSelection = (vendor: Vendor | undefined) => {
    setPurchaseForm(() => ({ ...purchaseForm, header: { ...purchaseForm.header, vendor } }));
    setOpenVendor(false);
  };
  ...
  const performUpdate = () => {
    ...
    put(`/purchases/${purchase.header.uuid}`, {
      ...headers,
    ...
    <AppLayout user={auth.user} breadcrumbs={makeEditBreadcrumbs(kind)}>
      <AppLayout.Actions>
        <div className="flex justify-end gap-x-6">
          ...
          <Button onClick={performUpdate} disabled={processing}>
