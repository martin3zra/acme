...
import type { Item, LineForm } from '@/types';
...
export const Lines = ({
  ...
  const handleCreateNewItem = () => {
    alert('this feature is not implemented yet. please create the item first and then add it to the purchase');
  };
  ...
  return (
    <>
      <table className="w-full table-auto">
        ...
        <tbody>
          {lines
            ...
            .map((line, index) => (
              <Line key={index} line={line} index={index} currency={currency} handleRemoveLine={handleRemoveLine} />
            ))}
        </tbody>
        {lineError && (
          <tfoot>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder={t('purchases.line.form.search')} value={typeof search === 'string' ? search : ''} onValueChange={setSearch} />
        <Command>
        ...
        <div className="flex w-full items-center justify-center rounded-b-lg border bg-gray-100/25 py-2">
          <button className="flex cursor-pointer items-center justify-center gap-x-2 text-indigo-400" onClick={handleCreateNewItem}>
            <LayoutListIcon className="size-4" /> {t('purchases.line.form.addNew')}
          </button>
        </div>
      </CommandDialog>
    </>
