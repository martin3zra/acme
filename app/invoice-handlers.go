...
func (s *Server) printInvoiceHandler(ctx *routing.Context) {
	...
	headers := []string{s.trans("global.item"), s.trans("global.unit"), s.trans("global.qty"), s.trans("global.price"), s.trans("global.tax"), s.trans("global.amount")}
	aligns := []string{"", "C", "C", "R", "R", "R"}
	widths := []float64{65, 15, 20, 25, 25, 30}
	...
	for _, line := range lines {
		...
		pdf.CellFormat(widths[0], 8, line.Name, "1", 0, "", false, 0, "")
		pdf.CellFormat(widths[1], 8, line.Unit.Name, "1", 0, "C", false, 0, "")
		pdf.CellFormat(widths[2], 8, strconv.FormatInt(line.Qty, 10), "1", 0, "C", false, 0, "")
		pdf.CellFormat(widths[3], 8, foundation.FormatAmount(line.Price), "1", 0, "R", false, 0, "")
		pdf.CellFormat(widths[4], 8, foundation.FormatAmount(line.Tax.Amount), "1", 0, "R", false, 0, "")
		pdf.CellFormat(widths[5], 8, foundation.FormatAmount(lineTotal), "1", 1, "R", false, 0, "")
	}
