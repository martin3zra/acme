...
func (s *Server) printInvoiceHandler(ctx *routing.Context) {
	...
	pdf.SetFooterFunc(func() {
		...
		footer := fmt.Sprintf("Page %d/{nb}", pdf.PageNo())
		pdf.CellFormat(
			10, 10, fmt.Sprintf("Generated by %s v0", s.config.appName), "", 0, "L", false, 0, "",
		)
		pdf.CellFormat(
	...
	pdf.AddPage()
	// 📛 Logo (if base64 provided)
	// if data.LogoBase64 != "" {
	logoBytes, err := base64.StdEncoding.DecodeString(logoBase64)
	if err == nil {
		pdf.RegisterImageOptionsReader("logo", fpdf.ImageOptions{
			ImageType: "PNG",
		}, bytes.NewReader(logoBytes))
		pdf.ImageOptions("logo", 15, 10, 30, 0, false, fpdf.ImageOptions{
			ImageType: "PNG",
		}, 0, "")
	}
	// }
	...
	pdf.SetXY(50, 25)
	pdf.CellFormat(80, 6, fmt.Sprintf("%s: %s", s.trans("companies.single.rnc_short"), company.Identifier), "", 1, "", false, 0, "")

	// Invoice Info (top right)
	pdf.SetXY(150, 10)
	pdf.CellFormat(0, 6, fmt.Sprintf("%s: %s", s.trans("global.invoice"), invoice.Number), "", 1, "R", false, 0, "")
	pdf.CellFormat(0, 6, fmt.Sprintf("NCF: %s", invoice.NCF), "", 1, "R", false, 0, "")
	pdf.CellFormat(0, 6, fmt.Sprintf("%s: %s", s.trans("global.date"), invoice.Date.Format("2006-01-02")), "", 1, "R", false, 0, "")
	...
	pdf.CellFormat(90, 6, strings.ToUpper(s.trans("global.billTo")), "", 0, "", false, 0, "")
	pdf.CellFormat(90, 6, strings.ToUpper(s.trans("global.paymentTerms")), "", 1, "", false, 0, "")
	pdf.SetFont("Arial", "", 10)

	pdf.CellFormat(90, 6, invoice.Customer.Name, "", 0, "", false, 0, "")
	if invoice.DueOn != nil {
		pdf.CellFormat(90, 6, fmt.Sprintf("%s %s", s.trans("global.dueDate"), invoice.DueOn.Format("2006-01-02")), "", 1, "", false, 0, "")
	} else {
		pdf.CellFormat(90, 6, strings.ToUpper(invoice.Terms), "", 1, "", false, 0, "")
	}
	pdf.CellFormat(90, 6, fmt.Sprintf("%s: %s", s.trans("global.email"), invoice.Customer.Email), "", 0, "", false, 0, "")
	if invoice.DueOn != nil {
		pdf.CellFormat(90, 6, fmt.Sprintf("%s: %s", s.trans("global.balance"), foundation.FormatAmount(invoice.AmountDue)), "", 1, "", false, 0, "")
	} else {
		pdf.Ln(6)
	}
	pdf.CellFormat(90, 6, fmt.Sprintf("%s: %s", s.trans("global.phone"), invoice.Customer.Phone), "", 0, "", false, 0, "")
	pdf.Ln(12)

	// Table Header
	pdf.SetFont("Arial", "B", 10)
	pdf.SetFillColor(240, 240, 255)
	headers := []string{s.trans("global.reference"), s.trans("global.item"), s.trans("global.unit"), s.trans("global.qtyAbrev"), s.trans("global.price"), s.trans("global.tax"), s.trans("global.amount")}
	...
	for _, line := range lines {
		...
		pdf.CellFormat(widths[4], 8, foundation.FormatAmount(line.Price), "1", 0, "R", false, 0, "")
		pdf.CellFormat(widths[5], 8, foundation.FormatAmount(line.Tax.Amount), "1", 0, "R", false, 0, "")
		pdf.CellFormat(widths[6], 8, foundation.FormatAmount(lineTotal), "1", 1, "R", false, 0, "")
	}
	...
	if strings.TrimSpace(invoice.Notes) != "" {
		// pdf.Ln(10)
		pdf.SetFont("Arial", "B", 10)
		pdf.CellFormat(labelW, 8, s.trans("global.notes"), "", 0, "L", false, 0, "")
		pdf.Ln(6)
		pdf.SetFont("Arial", "", 10)
		pdf.CellFormat(100, 8, invoice.Notes, "", 0, "L", false, 0, "")
	}

	pdf.SetY(y + 3)
	pdf.SetX(rightX - offset)
	pdf.CellFormat(labelW, 6, s.trans("global.subTotal"), "", 0, "R", false, 0, "")
	pdf.CellFormat(valW, 6, foundation.FormatAmount(subtotal), "", 1, "R", false, 0, "")

	discountAmount := invoice.Discount.Val
	discountPercentage := invoice.Discount.Val
	if invoice.Discount.Type == "percentage" {
		discountAmount = float64(invoice.Amount * (discountPercentage / 100))
	}
	if discountPercentage > 0 {
		pdf.CellFormat(130, 6, foundation.FormatAmount(discountPercentage, "%"), "", 0, "R", false, 0, "")
	}
	pdf.SetX(rightX - offset)
	pdf.CellFormat(labelW, 6, s.trans("global.discount"), "", 0, "R", false, 0, "")
	pdf.CellFormat(valW, 6, foundation.FormatAmount(discountAmount), "", 1, "R", false, 0, "")
	pdf.SetX(rightX - offset)
	pdf.CellFormat(labelW, 6, s.trans("global.tax"), "", 0, "R", false, 0, "")
	pdf.CellFormat(valW, 6, foundation.FormatAmount(totalTax), "", 1, "R", false, 0, "")
	pdf.SetFont("Arial", "B", 10)
	pdf.SetX(rightX - offset)
	pdf.SetFillColor(220, 240, 255) // light blue
	pdf.CellFormat(labelW, 8, s.trans("global.total"), "", 0, "R", false, 0, "")
	pdf.CellFormat(valW, 8, foundation.FormatAmount(subtotal+totalTax), "", 1, "R", false, 0, "")
	...
	if invoice.Status == InvoiceStatuses.Void {
		watermarkText = string(invoice.Status)
	}

	if invoice.PaidStatus == PaidStatuses.Paid {
		watermarkText = string(invoice.PaidStatus)
	...
	ctx.Response.Header().Set("Content-Disposition", fmt.Sprintf("inline; filename=%s.pdf", invoice.Number))
