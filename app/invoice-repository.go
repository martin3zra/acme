...
func invoiceMovementRecorded(ctx context.Context, sess playSession, companyID, invoiceID int) (bool, error) {
	var row invoiceModel
	if err := sess.Model(&invoiceModel{}).
...
func (s *Server) updateInvoice(ctx context.Context, uuid string, form *UpdateInvoiceForm) error {
	...
	return database.WithTransaction(s.db, func(tx *sql.Tx) error {
		...
		if _, err = ptx.Model(&invoiceModel{}).
			WhereEq("company_id", companyID).
...
func (s *Server) storeInvoiceInternal(tx *sql.Tx, companyID int, form *StoreInvoiceForm) (string, error) {
	...
	// maps uuid, which a struct insert would write as empty. The jsonb columns
	// carry the writer's already-encoded forms.
	invoiceID64, err := ptx.Model(&invoiceModel{}).Insert(context.Background(), map[string]any{
		"company_id":           companyID,
		"tax_receipt_id":       taxID,
		"tax_receipt_sequence": taxSeq,
	...
	invoiceUUID, err = playsql.RawScalar[string](ptx, context.Background(),
		"SELECT uuid FROM invoices WHERE company_id = $1 AND id = $2", companyID, invoiceID)
	if err != nil {
		return invoiceUUID, err
	}
