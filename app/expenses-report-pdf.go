...
func NewExpensesReportPDF(trans *i18n.Translator, form *ReportForm) (*ExpensesReportPDF, error) {
...
func (r *ExpensesReportPDF) AddLogo() {
...
func (r *ExpensesReportPDF) Header(ctx context.Context) {
...
func (r *ExpensesReportPDF) Footer(text string) {
...
func (r *ExpensesReportPDF) RenderReport(ctx context.Context, report []*expenseCategory, from, to time.Time, footerText string) {
	r.Header(ctx)
	r.Footer(footerText)

	if len(report) == 0 {
		r.RenderEmptyReport(r.trans.Trans("reports.pdf.expenses.title"))
		return
	}
	...
	r.pdf.CellFormat(0, 10, r.trans.Trans("reports.pdf.expenses.title"), "", 1, "L", false, 0, "")
	...
	for idx, row := range report {
		...
		r.pdf.CellFormat(155, 10, row.Name, "0", 0, "L", false, 0, "")
...
func (r *ExpensesReportPDF) RenderEmptyReport(title string) {
	if r.pdf.PageNo() == 0 {
		r.pdf.AddPage()
	}
...
func (r *ExpensesReportPDF) NeedsNewPage(requiredHeight, buffer float64) bool {
	y := r.pdf.GetY()
...
func (r *ExpensesReportPDF) ReserveSpace(requiredHeight, buffer float64) {
...
func (r *ExpensesReportPDF) UsableHeight() float64 {
